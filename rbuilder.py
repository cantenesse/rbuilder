...
def get_options():
    ...
    optparser.add_option("-a", "--application", action="store",
                         type="string", nargs=1)
    optparser.add_option("-d", "--build_dir", action="store",
                         type="string", nargs=1)
    ...
    optparser.add_option("-i", "--install_dir", action="store",
                         type="string", nargs=1)
    optparser.add_option("-x", "--rpmbuild_dir", action="store",
                         type="string", nargs=1)
...
def create_source_tar(rpmbuild_source, application, build_dir):
    ...
    tar = tarfile.open("%s/%s/%s.tar.gz" % (rpmbuild_source,
                                            application,
                                            application), "w:gz")
...
if __name__ == "__main__":
    ...
    (requires, application, git_repo,
     version, release, install_dir, rpmbuild_dir, arch) = get_options()
